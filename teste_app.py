...
st.metric = _noop
st.title = _noop
st.columns = lambda spec, **k: [_Col() for _ in (spec if isinstance(spec, list) else range(spec))]
...
try:
    exec(compile(open("app.py", encoding="utf-8").read(), "app.py", "exec"), {"__file__": "app.py"})
except Exception:
    print("FALHOU:"); traceback.print_exc(); sys.exit(1)

print(f"OK - {len(graficos)} graficos construidos e validados")
assert len(graficos) >= 6, f"esperava ao menos 6 graficos, obtive {len(graficos)}"
